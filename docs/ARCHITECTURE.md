...
Powered by ast-grep for language-aware AST parsing.
Used by MCP tools for AI-assisted development.
```
...
├───────────────────┼────────────────────────────────────────────┤
│ Symbol Overview   │ File structure with line ranges             │
│ Symbol Search     │ Find by name path (Struct/method)           │
│ References        │ Cross-file usage tracking                  │
│ Rename            │ Cross-file symbol rename                   │
│ Safe Editing      │ Optimistic locking via SHA256 content hash │
│ Pattern Search    │ Regex search with context                  │
└───────────────────┴────────────────────────────────────────────┘
```
...
2. Agent makes edits referencing hash
3. Edit only applies if hash matches current file
4. Prevents concurrent edit conflicts
```
...
```
Persistent cross-session knowledge for AI agents.
File-backed store surviving across conversations.
Enables continuity: decisions, context, feedback.
```
...
```
reflect     Synthesize memory landscape into actionable insights
checkpoint  Save session progress as timestamped memory
onboarding  Analyze project and bootstrap initial memory
```
...
30 tools across four categories:

```
┌────────────────────────────────────────────────────────────────┐
│ CATEGORY           │ TOOLS  │ FEATURE GATE                    │
├────────────────────┼────────┼─────────────────────────────────┤
│ Resource Mgmt      │ 8      │ always enabled                  │
│ Code Intelligence  │ 13     │ feature: code                   │
│ Memory Management  │ 7      │ feature: memory                 │
│ Onboarding         │ 2      │ code + memory                   │
└────────────────────┴────────┴─────────────────────────────────┘
```

Resource: `start_resource`, `stop_resource`, `restart_resource`, `get_status`, `list_resources`, `get_logs`, `start_all`, `stop_all`

Code: `get_symbols_overview`, `find_symbol`, `find_references`, `find_referencing_symbols`, `search_pattern`, `replace_symbol_body`, `insert_before_symbol`, `insert_after_symbol`, `insert_at_line`, `replace_lines`, `rename_symbol`, `find_file`, `list_dir`

Memory: `list_memories`, `read_memory`, `write_memory`, `edit_memory`, `delete_memory`, `reflect`, `checkpoint`

Meta: `onboarding`, `initial_instructions`
...
│   ├── config/             # ProjectConfig, ConfigStore, schema, discovery
│   ├── middleware/         # Middleware trait, MiddlewareStack, Operation
...
│   ├── types.rs            # Memory, MemoryKind, MemoryMeta
...
│   ├── retry.rs            # Auto-retry with backoff
│   └── timeout.rs          # Operation deadlines
│
